<template>
  <div class="flow-canvas" ref="canvasRef" @wheel.prevent="onWheel" @pointerdown="onCanvasPointerDown">
    ...
    <template v-else>
      ...
      <span class="text-[9px] text-arena-500 uppercase tracking-wider font-semibold px-1">Drag to add</span>
      ...
      <div class="flow-controls">
        <button @click="zoomIn" class="flow-ctrl-btn" title="Zoom in">+</button>
        <span class="text-[9px] text-arena-500 text-center select-none">{{ Math.round(scale * 100) }}%</span>
        <button @click="zoomOut" class="flow-ctrl-btn" title="Zoom out">−</button>
        <div class="border-t border-piedra-700/40 my-0.5"></div>
        <button @click="centerView" class="flow-ctrl-btn text-[9px]" title="Center view">Center</button>
      </div>
      </div>
    </template>
  </div>
