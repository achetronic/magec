...
|        | **Conversations**          | Projection over runs: `/conversations` (`?view=admin\|user`), `/conversations/{id}`, `/conversations/clear`, `/conversations/stats`, `/conversations/{id}/reset-session` |
...
- **MCP transports**: HTTP (`StreamableClientTransport` with optional headers + TLS skip) and stdio (`stdioCommandTransport`, a thin wrapper that builds a fresh `mcp.CommandTransport` per `Connect`). The wrapper exists because the ADK's mcptoolset reconnects on dead sessions and `exec.Cmd` is single-use — reusing one command across reconnects fails with "exec: Stdout already set" (issue #70). Stdio spawns subprocesses — works best with binary installs, not Docker
...
- **Flow editor**: `FlowCanvas.vue` (pan/zoom, SVG bezier edges, drag-to-connect ports with fan-out from non-router nodes, grouped add-node toolbar, full-screen toggle with double-Escape exit; minimized it shows a blurred preview with an "Edit in full screen" button) + `FlowNode.vue` (per-type node card with visible/renamable node ID chip, NodeHelp popover per type, resizable, positions/size persisted as node x/y/w/h). The router's fallback route is the fixed `otherwise` label, not a configurable field.
...
25. **Secrets in tool args expand via toolset wrapper, never adk's BeforeToolCallback**: the callback only offers in-place mutation of an args map shared with the model's functionCall event, which would leak expanded values into session history. `server/agent/secrets` wraps MCP toolsets and expands into a copy. See decision #38.
