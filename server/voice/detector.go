...
func (d *Detector) Load() error {
	d.logger.Info("Loading wake word models",
		"melspec", d.config.MelspecModelPath,
		"embedding", d.config.EmbeddingModelPath,
	...
	d.melspecSession, err = ort.NewDynamicAdvancedSession(
		d.config.MelspecModelPath,
		[]string{"input"},
		[]string{"output"},
		nil,
	)
	...
	d.embeddingSession, err = ort.NewDynamicAdvancedSession(
		d.config.EmbeddingModelPath,
		[]string{"input_1"},
		[]string{"conv2d_19"},
		nil,
	)
