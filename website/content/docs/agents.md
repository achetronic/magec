...
<div class="screenshots" style="margin-bottom: 2rem;">
{{< screenshot src="img/screenshots/admin-agent-dialog.png" alt="New Agent dialog" >}}
</div>
...
The backend dropdown shows all backends you've configured. The model is a free-text field. Type the model identifier exactly as the provider expects it.
...
[MCP (Model Context Protocol)](/docs/mcp/) lets you connect external tools to the agent: file access, web search, database queries, smart home control, GitHub operations, and hundreds more.
...
Skills are most useful when shared across agents. Update a product catalog skill once, and every agent that uses it sees the change.
...
Changes to agents take effect immediately. Edit a prompt, add a tool, change the voice, and save. The next message uses the new configuration. No server restart, no redeployment.
