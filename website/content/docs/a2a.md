...
In practice: you flip a toggle, and your agent gets a URL that other tools can connect to. They discover what the agent can do, authenticate, and send it messages, all without writing any code.
...
Agentic flows can also be exposed via A2A. The process is the same: open the flow in the editor, enable the A2A toggle, and save.
...
Discovery endpoints are public so that external clients can find your agents. The invocation endpoint requires a **Bearer token**: a regular Magec client token, the same ones you create in the Admin UI under Clients.
