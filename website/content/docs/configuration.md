...
## Seeds — Pre-built configurations

When Magec starts and finds an empty store, it can load a **seed** — a pre-built configuration that gives you a working setup out of the box. Seeds are controlled by the `MAGEC_SEED` environment variable, which the installer sets automatically based on your chosen deployment mode.

| Seed | Set by | What it includes |
|------|--------|-----------------|
| `voice-ui` | `--local` | Local Ollama backend, Redis session memory, PostgreSQL long-term memory, a default agent, and a Voice UI client |
| `voice-ui-openai` | `--openai` | OpenAI backend (using `${OPENAI_API_KEY}`), same memory and client setup |
| `voice-ui-anthropic` | `--anthropic` | Anthropic backend + local Ollama for embeddings, local Parakeet STT, local TTS |
| `voice-ui-gemini` | `--gemini` | Gemini backend + local Ollama for embeddings, local Parakeet STT, local TTS |

Seeds only apply on first run. Once you have a `store.json`, the seed is ignored — your configuration is yours to modify however you want.
